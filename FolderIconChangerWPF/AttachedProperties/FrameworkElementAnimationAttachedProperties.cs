...
namespace FolderIconChangerWPF.AttachedProperties
{
    ...
    /// and fading out on hide
    /// </summary>
    public class AnimateFadeInPropertyNoHitTestVisible : AnimateBaseProperty<AnimateFadeInProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value, bool firstLoad)
        {
            if (value)
                // Animate in
                await element.FadeInAsync(firstLoad, firstLoad ? 0 : 0.3f, true);
            else
                // Animate out
                await element.FadeOutAsync(firstLoad ? 0 : 0.3f, true);
        }
    }
